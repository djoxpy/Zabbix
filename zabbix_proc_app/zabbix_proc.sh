...
while [ -n "$1" ]; do
    ...
    -L)
        p="$2"
        if [ -n "$2" ]; then
            proc=$(top -bn1 | grep $p | sort -n -k1 | awk '{print "PID: "$1, "| "$NF}')
            IFS=$'\n'
            for var in $proc; do
                echo -e "\033[32m$var\e[0m"
            done
    ...
    -P)
        p="$2"
        if [ -n "$2" ]; then
            comm=$(sudo top -bn1 -w 200 | grep -w $p | awk '{print $9}')
            echo $comm
        else
            echo "No parameters found. Parameter is name of process. Use for example 'mysqld'"
        fi
    ...
    -R)
        p="$2"
        if [ -n "$2" ]; then
            comm=$(sudo top -bn1 -w 200 | grep -w $p | awk '{print $10}')
            echo $comm
        else
            echo "No parameters found. Parameter is name of process. Use for example 'mysqld'"
        fi
    ...
    -j)
        ...
        echo -e "["
        for ((c = 0; c < ${#customArray[@]} && c < ${#pidArray[@]}; c++)); do
    ...
    -J)
        p="$2"
        g=$(top -bn1 -w 200 | grep -w $p | awk '{print $NF}' ORS=',')
        #g=$(ps -e -T | grep $p | awk '{print $NF}' ORS=',') #alternative
        pid=$(top -bn1 -w 200 | grep -w $p | awk '{print $1}' ORS=',')
        IFS="," read -r -a customArray <<<$g
        IFS="," read -r -a pidArray <<<$pid
        echo -e "["
        for ((c = 0; c < ${#customArray[@]} && c < ${#pidArray[@]}; c++)); do
